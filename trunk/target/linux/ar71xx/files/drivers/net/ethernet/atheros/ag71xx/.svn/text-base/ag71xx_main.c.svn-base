...
static int gmac_hotplug_fill_event(struct bh_event *event)
{
	...
	ret = bh_event_add_var(event, 0, "GMAC=%s", event->name);
	if (ret)
		return ret;
...
static int gmac_hotplug_create_event(const char *dev_name, const u32 status)
{
	...
	event->name = dev_name;
	event->action = status ? "linkup" : "linkdown";
...
static void gmac_hotplug_event(const u8* dev_name, const u32 status)
{
	gmac_hotplug_create_event(dev_name, status);
}
#else
static void gmac_hotplug_event(const u8* dev_name, const u32 status)
{
...
void ag71xx_link_adjust(struct ag71xx *ag)
{
	...
	if (!ag->link) {
		...
		{
			pr_info("%s: link down\n", ag->dev->name);
			gmac_hotplug_event(ag->dev->name, 0);
		}
		return;
	}
	...
	if (netif_msg_link(ag))
	{
		...
		gmac_hotplug_event(ag->dev->name, 1);
	}
